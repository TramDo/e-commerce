...
<body>
	...
	<nav>
		<ul>
			<li><a href="${pageContext.request.contextPath}/Results.jsp">Home</a>
			</li>
			<li><a href="${pageContext.request.contextPath}/Results.jsp">About</a></li>
		</ul>
	</nav>
		...
		<fieldset>
			...
			<article>
				<c:if test="${not empty requestScope['flexible']}">
					<strong>Applicant Name: ${sessionScope['username']}</strong>
					<br />
					<BR />
				</c:if>
				...
				<br />
				<BR /> <strong>Monthly payments: </strong>
				<f:formatNumber type="currency">${payment}</f:formatNumber>
				<br />
				<br /> <small>Calculations are based on a fixed rate based
					on Prime Rate + 5%</small> <BR />
				<BR />
